...
xbm=rbm.*cos(tm);
ybm=rbm.*sin(tm);
h1=mesh(xbm,ybm,0.5.*ones(size(xbm)));
hold on
r2=2.*ones(1,11);
[rpm,tpm]=meshgrid(r2,theta);
xpm=rpm.*cos(tpm);
ypm=rpm.*sin(tpm);
h2=mesh(xpm,ypm,ones(size(xpm))*diag(0:0.05:0.5));
...
xpm=rpm.*cos(tpm);
ypm=rpm.*sin(tpm);
h=mesh(x+xpm,y+ypm,z-0.25+ones(size(xpm))*diag(0:0.05:0.5));
...
t(1)=mesh(xbm+x,ybm+y,z+0.25+zeros(size(xbm)));
t(2)=mesh(xbm+x,ybm+y,z-0.25+zeros(size(xbm)));
theta=pi:0.1.*pi:2*pi;
[rbm,tm]=meshgrid(rbase,theta);
xbm=rbm.*cos(tm);
ybm=rbm.*sin(tm);
t(3)=mesh(xbm+x,y+ybm,z+0.25+zeros(size(xbm)));
t(4)=mesh(xbm+x,y+ybm,z-0.25+zeros(size(xbm)));
yl=0:0.1:2;
xl=-0.5:0.1:0.5;
[xlm,ylm]=meshgrid(xl,yl);
t(5)=mesh(xlm+x,ylm+y,z+0.25+zeros(size(xlm)));
t(6)=mesh(xlm+x,ylm+y,z-0.25+zeros(size(xlm)));
[xlm,ylm]=meshgrid(xl,yl);
...
xbm=rbm.*cos(tm);
ybm=rbm.*sin(tm);
e(1)=mesh(xbm+x,ybm+y,z+zeros(size(xbm)));
e(2)=mesh(xbm+x,ybm+y,z-0.5+zeros(size(xbm)));
theta=0:0.1.*pi:pi;
rbase=0:0.05:0.25;
[rbm,tm]=meshgrid(rbase,theta);
xbm=rbm.*cos(tm);
ybm=rbm.*5.*sin(tm);
e(3)=mesh(xbm+x,ybm+y,z+0.25+zeros(size(xbm)));
e(4)=mesh(xbm+x,ybm+y,z-0.25+zeros(size(xbm)));
f=[e(3),e(4)];
